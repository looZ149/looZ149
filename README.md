<div align="center">

# Nils Klitsch

**Angehender Softwareentwickler · Fachinformatiker Anwendungsentwicklung (IHK)**

Linkenheim-Hochstetten, Deutschland &nbsp;·&nbsp;

</div>

---

## Über mich

Seit Februar 2025 absolviere ich eine Umschulung zum **Fachinformatiker für Anwendungsentwicklung** an der Lutz & Grub Academy in Karlsruhe. Meine betriebliche Praxisphase findet von April 2026 bis Februar 2027 statt.

Softwareentwicklung ist für mich mehr als Beruf – ich entwickle in meiner Freizeit eigene Projekte, arbeite an Open-Source-Software mit und bilde mich kontinuierlich weiter. Ich arbeite strukturiert, lerne schnell und gehe neue Herausforderungen mit Neugier und Ausdauer an.

---

## 🛠️ Technologien & Werkzeuge

**Sprachen**

![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white)
![C](https://img.shields.io/badge/C-555555?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-f34b7d?style=flat-square&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3572A5?style=flat-square&logo=python&logoColor=white)
![Assembly](https://img.shields.io/badge/x86_Assembly-6E4C13?style=flat-square&logoColor=white)
![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-E34F26?style=flat-square&logo=html5&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-f1e05a?style=flat-square&logo=javascript&logoColor=black)

**Frameworks & Bibliotheken**

![WPF](https://img.shields.io/badge/WPF-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![MAUI](https://img.shields.io/badge/MAUI-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![Torchvision](https://img.shields.io/badge/Torchvision-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)

---

## 🚀 Projekte

| Projekt | Beschreibung | Sprache |
|---|---|---|
| [**Network Monitor**](https://github.com/looZ149/Network-Monitor) | Netzwerk-Erkennungstool mit C# MAUI & MVVM – erkennt Geräte, liest MAC-Adressen aus und fragt Herstellerdaten per API ab | C# |
| [**JoF_EJK**](https://github.com/looZ149/JoF_EJK) | Community-Fork von EternalJK – Open-Source-Client für *Star Wars Jedi Knight: Jedi Academy* | C |
| [**PlantVision-AI**](https://github.com/looZ149/PlantVision-AI) | Bilderkennungs-KI zur Pflanzenerkennung auf Basis von Python, PyTorch & Torchvision | Python |
| [**Webserver x86-Assembly**](https://github.com/looZ149/Webserver-x86-assembly) | HTTP-Server vollständig in x86-Assembly – Systemprogrammierung auf unterster Ebene | Assembly |
| [**CBrowser**](https://github.com/looZ149/CBrowser) | Browser-Projekt in C++ zur Erkundung von Web-Rendering-Konzepten | C++ |

---

## 📊 GitHub Stats

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=looZ149&show_icons=true&theme=github_dark&hide_border=true&include_all_commits=true)
&nbsp;
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=looZ149&layout=compact&theme=github_dark&hide_border=true)

</div>

---

## 📜 Zertifikate

- **Basiszertifikat im Projektmanagement (GPM)** – PM-ZERT, gültig Feb 2026 – Feb 2031

---

